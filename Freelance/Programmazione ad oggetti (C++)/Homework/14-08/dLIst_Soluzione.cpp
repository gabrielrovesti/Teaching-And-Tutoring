...
class dList{
    ...
    /*
    dList<T> rende disponibile un opportuno overloading di operator< che implementa l'ordinamento lessicografico
    */
    ...
    bool operator<(const dList& dl) const{
        ...
        nodo* q = dl.first;
        while(p && q){
            if(p->info < q->info) return true;
            if(q->info < p->info) return false;
            p = p->next;
            q = q->next;
        }
        if(p) return false;

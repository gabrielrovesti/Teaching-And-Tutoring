...
public:
    ...
    virtual ~Workout(){}; //polimorfa = distruttore virtuale
    
    // metodi astratti/virtuali puri
    virtual Workout* clone() const = 0;
    virtual int calorie() const = 0;

    // uso "lampante" del const
...
public:
    ...
    // implementazione concreta (overriding dei metodi astratti)
    ...
    void removeNuoto(){
        ...
        for(auto i = vct.begin(); i < vct.end(); ++i){
            ...
            if(pippo && pippo->calorie() > maxCal){
                ...
                vct.erase(i);  // padre nostro della cancellazione 
